...
@section('content')
            ...
            <div class="card h-100 p-0 radius-12">
                ...
                <div class="card-body p-24" id="clientsTable">
                    @include('client.clientTable', ['clients' => $clients])
                </div>
            </div>
     ...
     <script>
         document.addEventListener('DOMContentLoaded', function () {
             ...
             input.addEventListener('keyup', function () {
                 ...
                 console.log(query);
                 if (query == '') {
                     fetch(`{{ route('client.search.zero') }}?q=${encodeURIComponent(query)}`)
                     ...
                     fetch(`{{ route('client.search') }}?q=${encodeURIComponent(query)}`)
                         .then(response => response.text())
                         .then(html => {
                             document.getElementById('clientsTable').innerHTML = html;
                         })
